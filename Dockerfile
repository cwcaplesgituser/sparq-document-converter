...
RUN apt-get update && apt-get install -y --no-install-recommends \
      ...
      default-jre-headless curl unzip ca-certificates python3-venv \
    && python3 -m venv /opt/worker-venv \
    && /opt/worker-venv/bin/pip install --no-cache-dir fastapi==0.116.1 uvicorn==0.35.0 python-multipart==0.0.20 \
