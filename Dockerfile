...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    && pip install --no-cache-dir fastapi==0.116.1 uvicorn==0.35.0 python-multipart==0.0.20 \
...
WORKDIR /work
EXPOSE 8080
CMD ["python", "-m", "uvicorn", "worker:app", "--app-dir", "/app", "--host", "0.0.0.0", "--port", "8080", "--workers", "1"]
