...
RUN apt-get update && apt-get install -y --no-install-recommends \
      ...
      default-jre-headless curl unzip ca-certificates python3-pip \
    && pip3 install --break-system-packages --no-cache-dir fastapi==0.116.1 uvicorn==0.35.0 python-multipart==0.0.20 \
...
WORKDIR /work
EXPOSE 8080
CMD ["python3", "-m", "uvicorn", "worker:app", "--app-dir", "/app", "--host", "0.0.0.0", "--port", "8080", "--workers", "1"]
